...
export default function ProcessoDetail({ processoId, onBack }: ProcessoDetailProps) {
  ...
  if (!proc) return <div className="text-slate-500">Processo não encontrado.</div>;
  ...
  return (
    <div>
      <button onClick={onBack} className="flex items-center gap-1 text-sm text-slate-500 hover:text-slate-900 mb-4 transition-colors">
        <ArrowLeft className="w-4 h-4" /> Processos
      </button>

      <div className="bg-white rounded-lg border border-slate-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          <h1 className="font-mono text-xl font-bold text-slate-900">{proc.numero_cnj || 'Sem número'}</h1>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2">
            <h2 className="text-sm font-semibold text-slate-900 mb-4 pb-2 border-b border-slate-100">Informações do Processo</h2>
          ...
          <div className="space-y-4">
            {resp && (
              <div className="bg-white border border-slate-200 rounded-lg p-4">
                <h3 className="text-xs text-slate-400 uppercase tracking-wide mb-3">Responsável</h3>
                <div className="flex items-center gap-3 mb-2">
                  <UserAvatar name={resp.name} color={resp.avatar_color} size="lg" />
                  <div>
                    <p className="text-sm font-medium text-slate-900">{resp.name}</p>
                    <p className="text-xs text-slate-400">{resp.oab}</p>
                  </div>
                </div>
                ...
                <p className="text-sm text-slate-500 mt-2">{resp.email}</p>
              </div>
            )}

            <div className="bg-slate-50 rounded-lg p-4">
              <h3 className="text-xs text-slate-400 uppercase tracking-wide mb-2">Observações</h3>
              <p className="text-sm text-slate-600 italic">{proc.notes || 'Nenhuma observação.'}</p>
            </div>
          </div>
        </div>
      </div>
