        ...
        <div className="col-span-4">
          <div className="bg-card border border-border rounded-lg shadow-sm h-fit sticky top-20">
                    ...
                    >
                      <div className="px-4 py-3">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          ...
                          <span className="text-xs text-muted-foreground">{evt.hora_inicio}{evt.hora_fim && evt.hora_fim !== evt.hora_inicio ? `–${evt.hora_fim}` : ''}</span>
                          {evt.cliente_nome && (
                            <>
                              <span className="text-muted-foreground">·</span>
                              <User className="w-3.5 h-3.5 text-muted-foreground" />
                              <span className="text-xs text-muted-foreground truncate">{evt.cliente_nome}</span>
                            </>
                          )}
                        </div>
                        {evt.local && (
                          <div className="mt-1 flex items-center">
                            <MapPin className="w-3 h-3 text-muted-foreground inline mr-1" />
                            <span className="text-xs text-muted-foreground truncate">{evt.local}</span>
                          </div>
                        )}
                        {lawyer && (
            ...
            <div className="border-t border-border px-5 py-3 bg-muted rounded-b-lg">
              <p className="text-xs font-semibold text-muted-foreground uppercase mb-2">Próximos eventos</p>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setShowDeleteConfirm(null)}>
          <div className="bg-card rounded-xl shadow-xl p-6 max-w-sm mx-4 text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-muted-foreground mt-1 mb-5">Esta ação não pode ser desfeita.</p>
            <div className="flex gap-3">
              <button onClick={() => setShowDeleteConfirm(null)} className="flex-1 border border-border rounded-md py-2 text-sm text-muted-foreground">Cancelar</button>
      ...
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col mx-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div className="flex items-center">
            ...
            <span className="text-lg font-semibold text-foreground">{editing ? 'Editar Evento' : 'Novo Evento'}</span>
          </div>
                ...
                {(['audiencia', 'reuniao', 'prazo', 'pericia', 'outro'] as EventoTipo[]).map((t) => (
                  <button
                    ...
                    className={`rounded-md px-3 py-2 text-sm border transition-colors ${
                      tipo === t ? tipoSelectedColors[t] : 'bg-card border-border text-muted-foreground'
                    }`}
                  >
                    {tipoLabels[t]}
                  </button>
...
function EventDetailModal({ evento, onClose, onEdit, onDelete }: EventDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      <div className="absolute inset-0 bg-black/20 backdrop-blur-sm" />
      <div className="relative bg-card rounded-xl shadow-xl w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className={`px-6 py-4 border-b border-border border-l-4 ${tipoBorderColors[evento.tipo]}`}>
          <div className="flex items-start justify-between">
            <div>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${tipoBgColors[evento.tipo]}`}>{tipoLabels[evento.tipo]}</span>
              <h3 className="text-lg font-semibold text-foreground mt-1">{evento.title}</h3>
            </div>
            <div className="flex gap-2">
              <button onClick={onEdit} className="text-muted-foreground hover:text-muted-foreground"><Edit className="w-4 h-4" /></button>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className="flex items-center text-sm text-foreground">
                <Calendar className="w-4 h-4 text-muted-foreground mr-1.5" />
            ...
            {processo && (
              <div className="col-span-2">
                <div className="flex items-center text-sm text-foreground">
                  <Briefcase className="w-4 h-4 text-muted-foreground mr-1.5" />
            ...
            {lawyer && (
              <div>
                <div className="flex items-center text-sm text-foreground gap-1.5">
                  <div className={`w-5 h-5 rounded-full ${lawyer.avatar_color} flex items-center justify-center text-white text-[9px] font-bold`}>
                    {lawyer.avatar_initials}
                  </div>
                  {lawyer.name}
            ...
            <>
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs text-muted-foreground uppercase">Observações</span>
                </div>
                <p className="text-sm text-muted-foreground italic leading-relaxed mt-1">{evento.notes}</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="px-6 py-4 border-t border-border flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} className="border border-border text-muted-foreground rounded-md px-4 py-2 text-sm">Fechar</button>
