...
const tipoDotColors: Record<string, string> = {
  ...
  documento_enviado: 'bg-slate-400',
...
const poloColors: Record<string, string> = {
  ...
  terceiro_interessado: 'bg-slate-50 text-slate-600',
};
...
export default function ClienteDetailPage({ clientId, onBack, onNavigateProcessoDetail }: ClienteDetailPageProps) {
  ...
  if (!cliente) {
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <h2 className="text-lg font-semibold text-slate-900 mb-1">Cliente não encontrado</h2>
  ...
  const prazoColor = prazoDiff <= 3 ? 'text-red-600' : prazoDiff <= 7 ? 'text-amber-600' : 'text-slate-900';
      ...
      <div className="border-l-2 border-slate-200 ml-3">
        {display.map((atv, i) => (
          <div key={atv.id} className={`relative pl-4 ${i < display.length - 1 ? 'pb-4' : ''}`}>
            <span className={`absolute -left-[5px] top-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${tipoDotColors[atv.tipo] || 'bg-slate-300'}`} />
            <div className="ml-1">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-400 mb-0.5">{tipoLabels[atv.tipo] || atv.tipo}</p>
              <p className="text-sm text-slate-700 leading-snug">{atv.descricao}</p>
              <div className="flex items-center gap-2 mt-1">
                <Clock className="w-3 h-3 text-slate-300" />
                <span className="text-xs text-slate-400">{formatDateBR(atv.data)} às {formatTimeBR(atv.data)}</span>
                <span className="text-slate-300">·</span>
                <span className="text-xs text-slate-400">{atv.usuario_nome}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200">
            <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Processo</th>
            <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Ação</th>
            {activeTab === 'processos' && (
              <>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Área</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Tribunal</th>
              </>
            )}
            ...
            {activeTab === 'processos' && (
              <>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Próx. Audiência</th>
                <th className="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-2.5">Prazo Fatal</th>
              </>
            )}
        ...
        <tbody>
          {procs.map((proc) => (
            <tr key={proc.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100 last:border-0">
              ...
              <td className="px-4 py-3">
                <div className="text-xs text-slate-600 truncate max-w-36">{proc.acao}</div>
                {activeTab === 'resumo' && (
              ...
              <td className="px-4 py-3 text-xs text-slate-400 truncate max-w-32">{proc.vara}</td>
              <td className="px-4 py-3">
              ...
              <td className="px-4 py-3">
                <div className="relative">
                  <button onClick={() => setOpenDropdown(openDropdown === proc.id ? null : proc.id)} className="text-slate-400 hover:text-slate-600 p-1">
                    <MoreHorizontal className="w-4 h-4" />
                  </button>
                  {openDropdown === proc.id && (
                    <div className="absolute right-0 top-8 bg-white border border-slate-200 shadow-lg rounded-lg py-1 z-10 w-44">
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 w-full" onClick={() => { setOpenDropdown(null); onNavigateProcessoDetail?.(proc.id); }}>
                        <Eye className="w-4 h-4" /> Ver Detalhes
                      </button>
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 w-full" onClick={() => setOpenDropdown(null)}>
                        <Scale className="w-4 h-4" /> Nova Audiência
                      </button>
                      <div className="my-1 border-t border-slate-100" />
                      <button className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 w-full" onClick={() => handleEncloseProcesso(proc.id)}>
  ...
  const renderDadosCadastrais = () => {
    ...
    fields.push({
      label: 'STATUS',
      value: (
        <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full ${cliente.status === 'ativo' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
    ...
    fields.push({
      ...
      value: responsible ? (
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-900">{responsible.name}</span>
        </div>
      ) : '—',
    });
    ...
    if (cliente.practice_area === 'criminal') {
      ...
      fields.push({
        ...
        value: (
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${situacaoDot[c.situacao_prisional] || 'bg-slate-300'}`} />
    ...
    if (cliente.practice_area === 'previdenciario') {
      ...
      fields.push({ label: 'CNIS', value: c.cnis_disponivel ? <span className="text-green-600">Disponível</span> : <span className="text-slate-400">Indisponível</span> });
      ...
      <div className="flex flex-col gap-3">
        {fields.map((f, i) => {
          ...
          if (f.label === '__SECTION__') {
            return <div key={i} className="border-t border-slate-100 my-1"><p className="text-xs text-slate-400 uppercase tracking-wide mt-2 mb-1">{f.value}</p></div>;
          }
          return (
            <div key={i}>
              <p className="text-xs text-slate-400 uppercase tracking-wide">{f.label}</p>
              <div className="text-sm font-medium text-slate-900 mt-0.5">{f.value}</div>
            </div>
          );
        })}
      </div>
  ...
  const selectCls = 'bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500';

  return (
    <div className="bg-slate-50 min-h-full -m-6">
      {/* Back nav */}
      <div className="px-6 pt-4 pb-0">
        <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900 cursor-pointer transition-colors">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-5 flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className={`w-14 h-14 rounded-full flex items-center justify-center font-semibold text-xl shrink-0 ${
            cliente.type === 'PF' ? 'bg-slate-200 text-slate-600' : 'bg-blue-100 text-blue-700'
          }`}>
            {initials}
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${cliente.type === 'PF' ? 'bg-slate-100 text-slate-600' : 'bg-blue-100 text-blue-700'}`}>
                {cliente.type}
              </span>
            </div>
            <div className="flex items-center gap-4 mt-1.5 flex-wrap text-sm text-slate-400">
              ...
              {polo && (
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${poloColors[polo] || 'bg-slate-50 text-slate-600'}`}>
                  {getPoloLabel(polo)}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => setSlideOpen(true)} className="border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
            <Edit className="w-4 h-4" /> Editar
          </button>
          <button className="border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
            <Mail className="w-4 h-4" /> E-mail
          </button>
          <button className="bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-md px-3 py-1.5 text-sm flex items-center gap-1.5 transition-colors">
          ...
          <div className="flex items-center gap-1.5 ml-2">
            <span className={`w-2 h-2 rounded-full ${cliente.status === 'ativo' ? 'bg-green-500' : 'bg-slate-400'}`} />
            <span className="text-xs text-slate-500 capitalize">{cliente.status}</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 bg-white border-b border-slate-200">
        <div className="grid grid-cols-4 gap-4">
          <div className="bg-slate-50 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-slate-900">{activeProcessCount}</p>
              <p className="text-xs text-slate-400">Processos Ativos</p>
            </div>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-slate-900">{nearestAudiencia ? formatDateShort(nearestAudiencia.proxima_audiencia) : '—'}</p>
              {nearestAudiencia && <p className="text-xs text-slate-400">{nearestAudiencia.tribunal}</p>}
              <p className="text-xs text-slate-400">Próxima Audiência</p>
            </div>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Prazo Fatal</p>
            </div>
          </div>
          <div className="bg-slate-50 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-slate-900">9.8</p>
              <p className="text-xs text-green-500">Excelência</p>
              <p className="text-xs text-slate-400">Satisfação NPS</p>
            </div>
          </div>
        </div>
      </div>
        ...
        {([
          ...
          >
            ...
            {'comingSoon' in tab && tab.comingSoon && (
              <span className="bg-slate-100 text-slate-400 text-xs px-1.5 py-0.5 rounded-full ml-1.5">Em breve</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="p-6">
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-7">
              <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
                <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-slate-900">Processos Recentes</h3>
            ...
            <div className="col-span-5 space-y-6">
              ...
              {cliente.notes ? (
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-lg px-5 py-4 flex items-start gap-3">
                  <StickyNote className="w-4 h-4 text-slate-300 mt-0.5 flex-shrink-0" />
        ...
        {activeTab === 'processos' && (
          <div className="bg-white rounded-lg border border-slate-200 shadow-sm">
            <div className="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-900">Processos ({relatedProcessos.length})</h3>
              <button className="bg-blue-600 text-white hover:bg-blue-700 rounded-md px-4 py-1.5 text-sm font-medium flex items-center gap-1.5 transition-colors">
                <Plus className="w-4 h-4" /> Novo Processo
              </button>
            </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              ...
              <div className="ml-auto">
                <button onClick={() => setAnnotationModal(true)} className="border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-md px-3 py-2 text-sm flex items-center gap-1.5 transition-colors">
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5">
              {filteredHistorico.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <h3 className="text-base font-semibold text-slate-900 mb-1">Nenhuma atividade registrada</h3>
                  <p className="text-sm text-slate-400">As atividades aparecerão aqui conforme forem registradas</p>
                </div>
              ) : (
        ...
        {activeTab === 'documentos' && (
          <div className="flex flex-col items-center justify-center gap-4 py-20">
            ...
            <h2 className="text-xl font-semibold text-slate-600">Módulo de Documentos</h2>
            <p className="text-sm text-slate-400 text-center max-w-xs">Este módulo estará disponível em breve.</p>
        ...
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm mx-4 text-center">
              ...
              <p className="text-sm text-slate-500 mt-1 mb-5">Esta ação não pode ser desfeita. O cliente será removido permanentemente do sistema.</p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirm(false)} className="flex-1 border border-slate-200 rounded-md py-2 text-sm text-slate-600 hover:bg-slate-50 transition-colors">Cancelar</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
              <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
                <h3 className="text-lg font-semibold text-slate-900">Nova Anotação</h3>
                <button onClick={() => setAnnotationModal(false)} className="text-slate-400 hover:text-slate-600"><X className="w-5 h-5" /></button>
              </div>
              <div className="px-6 py-4">
                <textarea
                  ...
                  className="w-full bg-white border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
              <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-3">
                <button onClick={() => setAnnotationModal(false)} className="text-sm text-slate-600 hover:text-slate-900 transition-colors">Cancelar</button>
